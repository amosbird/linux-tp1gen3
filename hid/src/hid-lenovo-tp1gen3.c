...
static int lenovo_input_mapping_tpx1gen3(struct hid_device* hdev, struct hid_input* hi,
...
static int lenovo_input_mapping(struct hid_device* hdev, struct hid_input* hi,
    struct hid_field* field, struct hid_usage* usage, unsigned long** bit, int* max) {
    switch (hdev->product) {
    case USB_DEVICE_ID_LENOVO_X1_TAB3:
        return lenovo_input_mapping_tpx1gen3(hdev, hi, field, usage, bit, max);
    default:
        return 0;
    }
}
...
static enum led_brightness lenovo_led_brightness_get_tpx1(struct led_classdev* led_cdev) {
    ...
    return drv_data->led_state & (1 << led) ? LED_FULL : LED_OFF;
}

static void lenovo_led_brightness_set_tpx1(
    ...
    if (usage->type == EV_KEY && usage->code == KEY_FN_ESC) {
        if (value == 1) {
            lenovo_led_brightness_set_tpx1(&drv_data->led_fnlock,
                (drv_data->led_state & (1 << HID_LENOVO_LED_FNLOCK) ? LED_OFF : LED_FULL));
            return 1;
        }
    }
    return 0;
...
static int lenovo_probe_tpx1gen3(struct hid_device* hdev) {
    ...
    if (!report) {
        ...
        drv_data = devm_kzalloc(&hdev->dev, sizeof(struct lenovo_drvdata_tpx1), GFP_KERNEL);
        if (!drv_data) {
